...
export let tFunN = (paramTypes, returnType) => ({
  type: "function",
  paramTypes,
  returnType,
});
...
export let inferExpr = (node, env, subst = {}) => {
  switch (node.type) {
    ...
    case "UnaryExpression":
      switch (node.operator) {
        ...
        case "-": {
          ...
          if (number.error)
            return error(
              unaryExpressionUnsupportedType(node, { types: [tNumber] }),
            );
          return ok(tNumber);
        }
        case "!": {
          ...
          if (boolean.error)
            return error(
              unaryExpressionUnsupportedType(node, { types: [tBoolean] }),
            );
          return ok(tBoolean);
        }
        default: {
          return error(
            unsupported(node, { stage: "inferExpr.UnaryExpression" }),
          );
        }
      }
    ...
    case "BinaryExpression":
      switch (node.operator) {
        ...
        case ">>>": {
          ...
          if (same.error)
            return error(binaryExpressionMismatch(node, { types: [tNumber] }));
          ...
          if (same.error)
            return error(
              binaryExpressionMismatch(node, {
                types: [tNumber, tString, tBoolean],
              }),
            );
          return ok(tBoolean);
        }
        default: {
          return error(
            unsupported(node, { stage: "inferExpr.BinaryExpression" }),
          );
        }
      }
    ...
    case "CallExpression": {
      ...
      const expectedFnType = tFunN(argTypes, returnType);
        ...
        if (fnType.value.type === "function") {
          ...
          if (actualArity !== expectedArity) {
            ...
            for (let i = 0; i < argTypes.length; i++) {
              let paramCheck = unify(
                argTypes[i],
                fnType.value.paramTypes[i],
                {},
              );
              if (paramCheck.error) {
                const fnName =
